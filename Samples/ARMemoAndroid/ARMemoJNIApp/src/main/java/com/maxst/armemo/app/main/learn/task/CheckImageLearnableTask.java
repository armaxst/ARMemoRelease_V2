...
public class CheckImageLearnableTask extends AsyncTask<Void, Void, CameraFrame> {
	...
	protected CameraFrame doInBackground(Void... noParams) {
		...
		while (tryCount++ < MAX_CHECK_COUNT) {
			...
			int result = ARMemoJNI.checkLearnable(cameraFrame.imageBuffer, cameraFrame.imageBuffer.length, cameraFrame.width, cameraFrame.height, cameraFrame.pixelFormat.getValue());
