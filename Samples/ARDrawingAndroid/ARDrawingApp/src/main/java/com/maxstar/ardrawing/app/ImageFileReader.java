...
public class ImageFileReader {
	...
	private static final class ImageReadHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			byte [] yuvData = ARDrawingUtils.readByteFromFile(fileName);

			// Read width (4bytes)
			int width = ARDrawingUtils.byteArrayToInt(yuvData, 0);

			// Read height (4bytes)
			int height = ARDrawingUtils.byteArrayToInt(yuvData, 4);
